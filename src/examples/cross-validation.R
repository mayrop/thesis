# The code will be based on blog post here
# http://www.milanor.net/blog/cross-validation-for-predictive-analytics-using-r/

# This is about cross validation in regression modelling
...
# Plotting the avg train error
lines(df, av_mse, lwd = 2, col = gray(0.4))

# Plotting the avg test error
lines(df, av_te, lwd = 2, col = "darkred")

# Avg points for train/test errors on 1 degree of freedom?
...
for (k in 1:n_folds) {
  ...
  x <- train_xy$x
  y <- train_xy$y
  fitted_models <- apply(t(df), 2, function(degf) {
    lm(y ~ ns(x, df = degf))
  })
  x <- test_xy$x
  y <- test_xy$y
  ...
  pred <- mapply(function(obj, degf) {
    predict(obj, data.frame(ns(x, df = degf)))
  }, fitted_models, df)
...
# Caret takes care of proportions in partitions
...
### Extra
pr <- prediction(pred_probs$good, cs_data_test$Status)
...
table(pred_classes, cs_data_test$Status)
mean(pred_classes == cs_data_test$Status)
